...
class OpenMEADeviceProcess:
    ...
    def initialize_device(self):
        print('Entered initialize_device function in process file', flush=True)
    ...
    def check_and_send_device_state(self):
        ...
        if device_state_str is None:
            self.emit_device_state({'isConnected': False})
            print('Device not connected', flush=True)
            return

        print('Device is Connected', flush=True)
        
        match_groups = re.match('([0-9]+),([0-9]+),([0-9a-f]+),([0-9]+),([01]),([01])', device_state_str.strip())
        print(f'Match Groups value: {match_groups}', flush=True)
        device_state_msg = {'isConnected': True}

        if not match_groups:
            device_state_msg['initState'] = 'NOT_INITIALIZED'
            print(f'Device state string: {device_state_str}', flush=True)

        elif (match_groups.group(5) != "0") or (match_groups.group(4) != "00000002"):
            device_state_msg['initState'] = "INIT_FAILED"

        else:
            device_state_msg['initState'] = 'INITIALIZED'
            device_state_msg['isSampling'] = match_groups.group(6) == "1"

            sample_duration_sclk = int(match_groups.group(2))
